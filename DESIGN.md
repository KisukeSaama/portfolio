...
- **One navigation, marked one way.** A header entry is a word with a rule that grows under it, and that holds on a phone too. The collapsed menu used to keep the grey rounded button the desktop nav had already dropped, and mark the current page down its left edge instead of under the word, so one navigation looked like two. Below 900 px it is a sheet across the full width of the header, not a card floating over one corner of it: at 360 px the card was a 19 rem island with the page showing down both sides.
- **The menu closes the way anything else closes.** A `<details>` element only ever closes from its own summary, so every press that meant "never mind" left the panel standing over the page. A press outside, Escape, and the viewport growing past the breakpoint all close it now, and a scrim dims what it covers so the press outside looks available.
- **Language and theme are rows, not icons parked on rows.** Inside the menu each is one full-width control naming the value it is currently on. They used to be a dead label with a 44 px icon button at the far end, so two thirds of something that looked pressable did nothing when pressed.
...
**The gutter is the one number that decides how close the page gets to the glass, and it never drops below 1.25 rem.** It used to fall to 1 rem under 680 px, which on a 360 px phone put the copy sixteen pixels from the edge. It also widens to clear a bezel: the site ships `viewport-fit: cover`, so a phone held sideways would otherwise run text under the notch. Nothing overrides it with a margin of its own. The contact panel did, with a negative inline margin that pulled it four pixels wider than the shell and, in doing so, replaced the `auto` that centres everything else: it sat flush against the left edge with the whole gap left over on the right.

**A phone held sideways is a separate case, not a narrow desktop.** It has width to spare and no height at all, so the menu pairs its entries into two columns rather than scrolling them under a fold nothing announces.

**A finger is not a cursor.** Hover states are declared inside `@media (hover: hover)`, because on a touch screen they stick to whatever was tapped last. Inline links whose target would otherwise be the height of their own text grow their hit area past the text under `@media (pointer: coarse)`, so nothing on the page moves and no line breaks differently.
