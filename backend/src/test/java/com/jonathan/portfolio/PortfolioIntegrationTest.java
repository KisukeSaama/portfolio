...
class PortfolioIntegrationTest {
    ...
    @Test void sessionEndpointAnswersAnonymousInsteadOfRefusing()throws Exception{mvc.perform(get("/api/v1/auth/session")).andExpect(status().isOk()).andExpect(jsonPath("$.authenticated").value(false));}
    ...
    @Test void adminCanCreatePublishArchiveAndRestore()throws Exception{var json=ProjectTestData.json("integration-project");var created=mvc.perform(post("/api/v1/admin/projects").with(user("admin@example.test").roles("ADMIN")).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(json)).andExpect(status().isCreated()).andReturn();var id=new tools.jackson.databind.ObjectMapper().readTree(created.getResponse().getContentAsString()).get("id").stringValue();mvc.perform(post("/api/v1/admin/projects/{id}/publish",id).with(user("admin@example.test").roles("ADMIN")).with(csrf())).andExpect(status().isOk()).andExpect(jsonPath("$.publicationStatus").value("PUBLISHED"));mvc.perform(post("/api/v1/admin/projects/{id}/archive",id).with(user("admin@example.test").roles("ADMIN")).with(csrf())).andExpect(status().isOk()).andExpect(jsonPath("$.publicationStatus").value("ARCHIVED"));mvc.perform(post("/api/v1/admin/projects/{id}/restore",id).with(user("admin@example.test").roles("ADMIN")).with(csrf())).andExpect(status().isOk()).andExpect(jsonPath("$.publicationStatus").value("DRAFT"));}
