...
public class MediaController {
    ...
    /**
     * Streams the file instead of buffering it: a 40 MB video would otherwise sit on the heap once
     * per concurrent request on an endpoint anyone can call. {@code Content-Disposition: inline}
     * with a fixed name keeps the browser from treating an unexpected body as a download the
     * stored filename gets to title.
     */
    @GetMapping("/api/v1/public/media/{id}") public ResponseEntity<org.springframework.core.io.Resource> publicMedia(@PathVariable UUID id){
        var media=service.publicObject(id);
        return ResponseEntity.ok()
            ...
            .body(new org.springframework.core.io.FileSystemResource(media.path()));
    }
}
