...
public class ProjectService {
    ...
    public ProjectResponse create(ProjectWriteRequest input,Authentication auth,HttpServletRequest request){
        validateContent(input); if(projects.existsBySlugIgnoreCase(input.slug())) throw new ConflictException("This slug is already in use.");
    ...
    public ProjectResponse update(UUID id,ProjectWriteRequest input,Authentication auth,HttpServletRequest request){
        validateContent(input); var project=find(id); if(projects.existsBySlugIgnoreCaseAndIdNot(input.slug(),id)) throw new ConflictException("This slug is already in use.");
    ...
    @Transactional
    public void delete(UUID id,String confirmation,Authentication auth,HttpServletRequest request){var project=find(id);if(project.getPublicationStatus()!=PublicationStatus.ARCHIVED||!project.getTitle().equals(confirmation))throw new InvalidOperationException("Archive the project and type its exact title before permanent deletion.");audit.record(AuditAction.PROJECT_DELETE,currentAdmin.id(auth),project,"{\"title\":\""+escape(project.getTitle())+"\"}",request);projects.delete(project);}
    private Project find(UUID id){return projects.findById(id).orElseThrow(()->new NotFoundException("Project not found."));}
    private String uniqueCopySlug(String source){var base=source+"-copy";var slug=base;var suffix=2;while(projects.existsBySlugIgnoreCase(slug))slug=base+"-"+suffix++;return slug;}
    private void validateContent(ProjectWriteRequest r){var strings=new ArrayList<String>(List.of(r.title(),r.shortDescription(),r.fullDescription(),r.problem(),r.context(),r.solution(),r.role(),r.architecture()));strings.addAll(r.objectives());strings.addAll(r.features());strings.addAll(r.decisions());strings.addAll(r.challenges());strings.addAll(r.learnings());strings.addAll(r.nextSteps());if(strings.stream().anyMatch(v->v.contains("<")||v.contains(">")))throw new InvalidOperationException("HTML content is not accepted. Use plain text.");}
