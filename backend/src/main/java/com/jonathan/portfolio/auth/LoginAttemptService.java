...
public class LoginAttemptService {
    ...
    public String key(String email,String ip){try{var digest=MessageDigest.getInstance("SHA-256");return HexFormat.of().formatHex(digest.digest((ip+":"+email.toLowerCase()).getBytes(StandardCharsets.UTF_8)));}catch(Exception ex){throw new IllegalStateException("Hash indisponible",ex);}}
